...
def buildYoutube(user):
  credentials = getCredentials(user)
  return build('youtube', 'v3', credentials=credentials)


def getCredentials(user):
    Google = SocialApp.objects.all().filter(provider="Google").first()
    socialaccount = SocialAccount.objects.all().filter(provider="google", user_id=user.id).first()
    socialtoken = SocialToken.objects.all().filter(account_id=socialaccount.id).first()
    creds = Credentials(
        socialtoken.token,
